...
struct SnowDaysChart: View {
    ...
    private var maxSnowDays: Double {
        Double(sortedData.map { $0.snowDays }.max() ?? 100)
    }
    
    private var minSnowDays: Double {
        Double(sortedData.map { $0.snowDays }.min() ?? 0)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.md) {
            ...
            HStack {
                ...
                Text("snow_days_trend".localized)
                    .font(DesignSystem.Typography.headline)
                    .fontWeight(.bold)
            ...
            if !sortedData.isEmpty {
                ...
                VStack(spacing: DesignSystem.Spacing.sm) {
                    ...
                    GeometryReader { geometry in
                        ZStack {
                            ...
                            // Gradient Fill
                            snowDaysFill(in: geometry.size)
                                .fill(
                                    LinearGradient(
                                        colors: [
                                            DesignSystem.Colors.info.opacity(0.3),
                ...
                HStack {
                    VStack(alignment: .leading, spacing: DesignSystem.Spacing.xs) {
                        ...
                        HStack {
                            Text("range".localized)
                                .font(DesignSystem.Typography.caption2)
                                .foregroundColor(DesignSystem.Colors.secondaryText)
                            Text("\(Int(minSnowDays)) - \(Int(maxSnowDays)) \("days".localized)")
                                .font(DesignSystem.Typography.caption2)
                                .fontWeight(.medium)
                                .foregroundColor(DesignSystem.Colors.primaryText)
                        }
                    ...
                    if sortedData.count >= 2 {
                        let trend = Double(sortedData.last!.snowDays - sortedData.first!.snowDays)
    ...
    private func snowDaysLine(in size: CGSize) -> Path {
        Path { path in
            guard !sortedData.isEmpty else { return }
            
            let stepX = size.width / max(1, CGFloat(sortedData.count - 1))
            let range = maxSnowDays - minSnowDays
            
            // Calculate all points first for smooth curves
            var points: [CGPoint] = []
            for (index, yearData) in sortedData.enumerated() {
                let x = CGFloat(index) * stepX
                let normalizedValue = range > 0 ? (Double(yearData.snowDays) - minSnowDays) / range : 0
    ...
    private func snowDaysFill(in size: CGSize) -> Path {
        Path { path in
            guard !sortedData.isEmpty else { return }
            ...
            if let firstData = sortedData.first {
                let normalizedValue = range > 0 ? (Double(firstData.snowDays) - minSnowDays) / range : 0
                let y = size.height - (CGFloat(normalizedValue) * size.height)
        ...
        ZStack {
            ForEach(Array(sortedData.enumerated()), id: \.offset) { index, yearData in
                ...
                Circle()
                    ...
                    .position(x: x, y: y)
                    .shadow(color: DesignSystem.Colors.info.opacity(0.5), radius: 4)
            }
        }
    }
}
...
#Preview {
    ...
    ZStack {
        ...
        ScrollView {
            SnowDaysChart(yearlyData: sampleData)
                .padding()
        }
    }
